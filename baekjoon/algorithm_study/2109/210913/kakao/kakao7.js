...
function solution(board, aloc, bloc) {
  ...
  board = [Array(len).fill(0)]
      ...
      .map(e => [0].concat(e).concat([0]));
  const DFS = (turn, cnt) => {
      let moved = false;
      let curr = turn == 0 ? aloc : bloc;
      if (board[curr[0]][curr[1]] == 0) {
          if (bmax < Math.floor(cnt / 2)) {
              bmax = Math.floor(cnt / 2);
              answer = cnt;
          }
          return;
      }
      ...
      for (let k = 0; k < 4; k++) {
          ...
          if (board[ny][nx]) {
              moved = true;
              if (turn == 0) aloc = [ny, nx];
              else bloc = [ny, nx];
              DFS((turn + 1) % 2, cnt + 1);
              if (turn == 0) aloc = [curr[0], curr[1]];
              else bloc = [curr[0], curr[1]];
          }
      }
      if (moved == false && cnt < answer && bmax <= Math.floor(cnt / 2)) {
          answer = cnt;
          bmax = Math.floor(cnt / 2);
      }
  }
